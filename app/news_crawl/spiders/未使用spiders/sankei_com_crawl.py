...
class SankeiComCrawlSpider(ExtensionsCrawlSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not "category_urls" in kwargs:
            raise CloseSpider("引数エラー：当スパイダー(" + self.name + ")の場合、category_urlsは必須です。")
    ...
    def start_requests(self):
        ...
        urls: list = ["https://www.sankei.com/%s" % str(i) + "/" for i in category_urls]
    ...
    def parse_start_response(self, response: Response):
        ...
        while page <= page_to:  # 条件はあとで考える
            ...
            self.logger.info(f"=== parse_start_response ページ内リンク件数 = {len(elements)}")
            ...
            self.logger.info(f"=== parse_start_response クロール対象リンク件数 = {len(urls_list)}")
            ...
            # 次のページを読み込む
            elem: WebElement = driver.find_element_by_css_selector(".feedPagination")
            elem.click()
