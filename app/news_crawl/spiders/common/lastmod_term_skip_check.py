...
class LastmodTermSkipCheck:
    lastmod_term_datetime_from: datetime | None = None
    lastmod_term_datetime_to: datetime | None = None
    ...
    def __init__(
        ...
        spider,
        start_time: datetime,
        lastmod_term_minutes_from: int | None,
        lastmod_term_minutes_to: int | None,
    ) -> None:
        """
        lastmodの期間指定がある場合、datetime形式にしてクラス変数に保存
        """
        if lastmod_term_minutes_from or lastmod_term_minutes_to:
            if lastmod_term_minutes_from is not None:
                self.lastmod_term_datetime_from = start_time - timedelta(minutes=lastmod_term_minutes_from)
                spider.logger.info(
                    f"=== lastmod_term_minutes_from より計算した時間: {self.lastmod_term_datetime_from.isoformat()}"
                )
            if lastmod_term_minutes_to is not None:
                self.lastmod_term_datetime_to = start_time - timedelta(minutes=lastmod_term_minutes_to)
                spider.logger.info(
