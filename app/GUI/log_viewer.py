...
class LogViewer(tkinter.Frame):
    ...
    def init_screen_create(self):
        ...
        # 初画面表示の項目を定義
        # start_time_from
        date_from_label = tkinter.LabelFrame(self, text="start_time(from)")
        self.date_from = tkinter.Entry(date_from_label)
        self.time_from = tkinter.Entry(
            date_from_label,
            width=8,
        )
        ...
        # start_time_to
        date_to_label = tkinter.LabelFrame(self, text="start_time(to)")
        ...
        record_type_list = [str(x["class_name"]) for x in directory_search_task()]
        ...
        # ログメッセージレベルフィルター
        self.log_level_label = tkinter.LabelFrame(self, text="ログレベルフィルター")
        ...
        # 総件数
        self.record_count_label_frame = tkinter.LabelFrame(self.info_box, text="総件数")
        self.record_count_label_frame.grid(row=0, column=0)
        self.record_count_label = tkinter.Label(
            self.record_count_label_frame,
        ...
        # 総ページ数
        self.page_count_label_frame = tkinter.LabelFrame(self.info_box, text="総ページ数")
        self.page_count_label_frame.grid(row=0, column=1)
        self.page_count_label = tkinter.Label(
            self.page_count_label_frame,
            textvariable=self.max_page_count,
            width=6,
            font=font1,
        )
        ...
        self.first_page_button = tkinter.Button(
            ...
            width=2,
            state="disabled",
            command=lambda: self.logs_edit("first"),
        )
        ...
        self.previous_page_button = tkinter.Button(
            ...
            width=2,
            state="disabled",
            command=lambda: self.logs_edit("-1"),
        )
        self.previous_page_button.grid(row=0, column=1)
        self.current_page_button = tkinter.Button(
            self.page_selecter_label_frame,
            text=str(self.current_page.get()),
            width=2,
            state="disabled",
            command="",
        )
        ...
        self.last_page_button = tkinter.Button(
            ...
            width=2,
            state="disabled",
            command=lambda: self.logs_edit("last"),
        )
    ...
    def log_list_view(self):
        ...
        try:
            ...
            # 小数点以下切り上げ
            self.max_page_count.set(-(-self.record_count.get() // self.number_of_lines))
            ...
            if self.record_count.get():
                ...
                self.logs_frame.grid(row=5, column=0, columnspan=4, sticky=tkinter.W)
    ...
    def log_view(self, mongo_id):
        ...
        for record_key, record_value in record.items():
            # 項目名、コピーボタン
            item_name = tkinter.Entry(
                log_window,
            )
            item_name.insert(tkinter.END, str(record_key))
            item_name.grid(row=row_pointer, column=0, sticky=tkinter.NW, ipadx=30)
            ...
            if record_key == "logs":
                ...
                item_value.insert(tkinter.END, str(record_value))
                item_value.grid(row=row_pointer, column=1, sticky=tkinter.EW, ipadx=500)
                # 次の行ポインターへ
                row_pointer += 1
    ...
    def logs_list_get(self, page: int, record_count: int) -> tuple[Cursor, int]:
        ...
        record = (
            self.crawler_log.find(
                filter=self.filter,
                sort=[("start_time", DESCENDING)],
            )
            .skip(skip_list[page - 1])
            .limit(limit)
        )
    ...
    def log_get(self, mondo_id: str) -> Any:
        """ログを１件取得"""
        return self.crawler_log.find_one(
            filter={"_id": mondo_id},
        )
