...
def stats_analysis_report_flow(report_term: str, totalling_term: str, base_date: date | None = None):
    init_flow()
    ...
    if init_task_instance.state.is_completed():
        ...
        try:
            ...
            stats_analysis_report_input = stats_analysis_report_args_check_task(report_term, totalling_term, base_date)

            # スクレイパー情報解析用のデータフレーム管理クラスを生成
            stats_info_collect_data = stats_analysis_report_data_frame_task(mongo, stats_analysis_report_input)
            ...
            if stats_info_collect_data:
                ...
                stats_analysis_report_notice_task(stats_analysis_report_input, stats_analysis_report_excel)

            else:
                # 対象データがない場合
                logger.warning("=== 該当データが無いため「スクレイパー情報解析レポート」の作成をキャンセルしました。")
