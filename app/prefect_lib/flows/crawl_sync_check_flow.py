...
def crawl_sync_check_flow(domain: str, start_time_from: datetime, start_time_to: datetime):
    ...
    if init_task_result.get_state().is_completed():
        ...
        try:
            ...
            response_sync_list, response_async_list, response_async_domain_aggregate = sync_check_crawler_response_task(mongo, domain, start_time_from, start_time_to)

            # crawler_responseとnews_clip_masterが同期しているかチェックする。
            master_sync_list, master_async_list, master_async_domain_aggregate = sync_check_news_clip_master_task(mongo, domain, start_time_from, start_time_to, response_sync_list)

            # 当分の間solrとの同期チェックは中止。solr側の本格開発が始まってから開放予定。
            #solr_sync_list, solr_async_list, solr_async_domain_aggregate = sync_check_solr_news_clip(mongo, domain, start_time_from, start_time_to, master_sync_list)
            solr_async_list = []
