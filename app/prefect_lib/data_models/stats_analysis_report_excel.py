...
class StatsAnalysisReportExcel:
    ...
    stats_analysis_columns_info: list = [
        ...
        {HEAD1: '', HEAD2: '最大', COL: 'memusage/max_max',
         DIGIT_ADJUSTMENT: 1000,
         WARNING_VALUE_OVER: 500000000, WARNING_BACKGROUND_COLOR: 'FF8C00'},     # メモリ使用量の500mbを超えている場合ワーニング
    ...
    def stats_analysis_report_body(self, spider_result_all_df: pd.DataFrame, spider_list: pd.Series, ):
        '''
        統計情報解析レポート用Excelの編集。
            ...
            for col_idx, col_info in enumerate(columns_info_by_spider):
                for row_idx, value in enumerate(by_spider_df[col_info[self.COL]]):
                    ...
                    if self.EQUIVALENT_COLOR in col_info:
                        ...
                        any: Any = self.worksheet_1[f'{get_column_letter(col_idx + 1)}{str(base_row_idx + row_idx - 1)}']
        ...
        max_cell: str = get_column_letter(
            self.worksheet_1.max_column) + str(self.worksheet_1.max_row)  # "BC55"のようなセル番地を生成
        ...
        for spider in spider_list:
            ...
            for col_idx, col_info in enumerate(columns_info_by_spider):
                ...
                for row_idx, value in enumerate(by_spider_df[col_info[self.COL]]):
                    ...
                    if self.WARNING_VALUE in col_info:
                        # if value in col_info[self.WARNING_VALUE]:
                        if value and value != '200':            # （暫定対応）ステータスに値があり、かつ、200以外はワーニングとする。
                            target_cell.fill = PatternFill(
                                patternType='solid', fgColor=col_info[self.WARNING_BACKGROUND_COLOR])
                            warning_flg = True
