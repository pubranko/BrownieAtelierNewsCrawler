                    ...
                    [
                        {
                            ...
                            self.TIME_PERIOD_HOUR: start_time.strftime("%H"),
                            self.SPIDER_NAME: spider_name,
                            self.DOWNLOADER_RESPONSE_STATUS: str(key).replace(
                                self.DOWNLOADER_RESPONSE_STATUS_COUNT, ""
                            ),
                            self.COUNT: value,
                        }
                    ]
    ...
    def stats_analysis_exec(
        ...
        for calc_date_from, calc_date_to in datetime_term_list:
            ...
            self.aggregate_result_set(
                robots_select_df,
                [self.SPIDER_NAME, self.ROBOTS_RESPONSE_STATUS],
                date_from,
                self.robots_result_df,
            )
            self.aggregate_result_set(
                downloader_select_df,
                [self.SPIDER_NAME, self.DOWNLOADER_RESPONSE_STATUS],
                date_from,
        ...
        df_sort_list: list[tuple[dict[str, pd.DataFrame], list]] = [
            ...
            (self.spider_result_df, [self.SPIDER_NAME, self.AGGREGATE_BASE_TERM]),
        ]
        for type in [
            self.AGGREGATE_TYPE__SUM,
            self.AGGREGATE_TYPE__MEAN,
            self.AGGREGATE_TYPE__MIN,
            self.AGGREGATE_TYPE__MAX,
        ]:
        ...
        spider_result_all_df = pd.merge(
            self.spider_result_df[self.AGGREGATE_TYPE__SUM],
            self.spider_result_df[self.AGGREGATE_TYPE__MEAN],
            suffixes=["", "_mean"],
            on=[self.AGGREGATE_BASE_TERM, self.SPIDER_NAME],
        )
        spider_result_all_df = pd.merge(
            spider_result_all_df,
            self.spider_result_df[self.AGGREGATE_TYPE__MIN],
            suffixes=["", "_min"],
            on=[self.AGGREGATE_BASE_TERM, self.SPIDER_NAME],
        )
        spider_result_all_df = pd.merge(
            spider_result_all_df,
            self.spider_result_df[self.AGGREGATE_TYPE__MAX],
            suffixes=["", "_max"],
            on=[self.AGGREGATE_BASE_TERM, self.SPIDER_NAME],
        )
