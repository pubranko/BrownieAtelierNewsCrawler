...
def morning_flow_net(
    ...
    if base_datetime:
        ...
        today = (
            datetime.now().astimezone(TIMEZONE).replace(minute=0, second=0, microsecond=0)
        )  # 当日の現在時・０分・０秒
    ...
    mongo_delete_selector_flow(
        ...
        period_date_from=(
            datetime.now().astimezone(TIMEZONE) - relativedelta(months=1)
        ).date(),  # 消し忘れ防止用に直近１か月間のデータを削除
