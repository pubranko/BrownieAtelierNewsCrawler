...
def mongo_delete_task(
    ...
    for collection_name in collections_name:
        ...
        if collection_name == CrawlerResponseModel.COLLECTION_NAME:
            ...
            if crawler_response__registered:
                conditions.append({
                    CrawlerResponseModel.NEWS_CLIP_MASTER_REGISTER: {
                        '$exists': True}   # news_clip_masterへの登録処理を実施済みのレコードのみ削除する。
                })
            ...
            conditions.append(
                {CrawlerLogsModel.START_TIME: {'$lte': period_to}})
            ...
            conditions.append(
                {AsynchronousReportModel.START_TIME: {'$lte': period_to}})
            ...
            conditions.append(
                {StatsInfoCollectModel.START_TIME: {'$lte': period_to}})
        ...
        if collection:
            ...
            if collection_name == CrawlerResponseModel.COLLECTION_NAME:
                if conditions_complete:
                    ...
                    delete_count_skip: int = collection.count(
                        filter=filter_skip)
