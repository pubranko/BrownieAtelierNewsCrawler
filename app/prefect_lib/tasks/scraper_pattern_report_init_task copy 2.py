...
@task
def scraper_pattern_report_init_task(
    mongo: MongoModel,
    report_term: str,
    base_date: Optional[datetime] = None
    ) -> Tuple[ScraperPatternReportInput, ScraperPatternReportData]:
    '''
    ・入力（Flowの引数）のバリデーションチェック。
    ・ドメイン別にニュースクリップマスターより対象期間のレコード件数を確認。
    ・上記で取得したレコードを元に統計用のデータフレームを生成。
    ・戻り値: 入力データクラス、スクレイパー情報解析用のデータフレーム管理クラス
    '''
    ...
    logger.info(
        f'=== ScrapyingPatternReportTask run 基準日from ~ to : {scraper_pattern_report_input.base_date_get()}')
    ...
    return scraper_pattern_report_input, scraper_pattern_report_data
